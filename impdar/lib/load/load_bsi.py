...
#def _dt_from_comment(dset):
    ...
    #print(low_level_group.id)		#KI
    #print(group_comment, ' + empty')		#KI
    #print(dset.attrs['CreationTimestamp'])		#KI
...
# ----- KI ------
# Updated initial date of data collection from the comment 

def _dt_from_comment(dset):
	"""Return the day when the first location in the dset was collected."""
	tc = dset.attrs['CreationTimestamp']
	tc = tc[:tc.find(' ')]
	dmy =  list(map(int,tc.split('/')))
	return datetime.datetime(dmy[2], dmy[0], dmy[1], 0, 0, 0)
...
def load_bsi(fn_h5, nans=None, *args, **kwargs):
    ...
    with h5py.File(fn_h5, 'r') as f_in:
        ...
        for dset_name in dset_names:
            ...
            #h5_data.tnum = len(dset.keys())	# number of traces
            h5_data.snum = len(
                dset['location_0']['datacapture_0']['echogram_0']) # number of samples per trace
            ...
            for location_num in range(h5_data.tnum):
                ...
                if (float(_xmlGetVal(gps_data, 'GPS Fix valid')) > 0) and (
                    ...
                    try:
                        lat[location_num] = float(_xmlGetVal(gps_data, 'Lat'))  # KI --- changed from Lat_N
                        lon[location_num] = float(
                            _xmlGetVal(gps_data, 'Long')) # KI --- changed from Long_W
