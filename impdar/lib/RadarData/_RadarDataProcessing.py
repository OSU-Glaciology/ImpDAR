...
def hcrop_ki(self, lim_left, lim_right):
    """Crop the radar data in the horizontal in a window. We can take off a window of traces.
    ...
    ----------
    lim_left: int
        Left boundary (trace) to crop
    lim_right: str
        Right boundary (trace) to crop
    """
    lim = [lim_left,lim_right]
    wind = lim[1]-lim[0]
    if wind > self.tnum:
        raise ValueError('lim should be less than tnum+1 {:d} in order to do anything'.format(self.tnum + 1))
    if wind <= 0:
        raise ValueError('Distance should be strictly positive. Check left and right input')
    ...
    # Most variable just need subsetting
    self.data = self.data[:, mask[0,:]]
    for var in ['lat', 'long', 'pressure', 'trace_int', 'trig', 'elev', 'x_coord', 'y_coord', 'decday']:
        # some of these are optional, and trig may be set to a float rather than an array
        if getattr(self, var) is not None and isinstance(getattr(self, var), np.ndarray):
            setattr(self, var, np.append(getattr(self, var)[:lim[0]],getattr(self, var)[lim[1]:]))

    if self.picks is not None:
        self.picks.hcrop_ki(lim[0],lim[1])

    # More complex modifications for these two
    if self.dist is not None:
        self.dist = np.append(self.dist[:lim[0]], self.dist[lim[1]:] - lim[0])
    self.trace_num = np.append(self.trace_num[:lim[0]], self.trace_num[lim[1]:] - lim[0]) + 1
